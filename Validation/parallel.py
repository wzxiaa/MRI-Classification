...
#         print("P_X separate", P_X)
    ...
    def clusteringP(self, P_X):
        ...
        """
        affinity_networks = compute.make_affinity(
            P_X, metric='euclidean', K=self.numP, mu=0.5)
    ...
    def clusteringS(self, S_X):
        ...
        affinity_networks = compute.make_affinity(
            S_X, metric='euclidean', K=self.numS, mu=0.5)
...
def integrated_grid_search(
    X, Y, subtype_label, dataset, train_index, test_index, fold,
    ...
    X_train_harmonized = X_train
    X_test_harmonized = X_test
    ...
    if dataset == 'c' or dataset == 'i':
        ...
        X_train_harmonized_without_age_sex = X_train_harmonized
        ...
        X_train_ttest_with_age_sex, columns_ttest = t_test(
            X_train_harmonized_without_age_sex, y_train)

        # X_train_ttest_with_age_sex = np.concatenate(
        #     (X_train_ttest_without_age_sex, train_age_sex_data), axis=1)
        ...
        X_test_harmonized_without_age_sex = X_test_harmonized
        X_test_ttest_without_age_sex = X_test_harmonized_without_age_sex[:, columns_ttest]
        # X_test_ttest_with_age_sex = np.concatenate(
        #     (X_test_ttest_without_age_sex, test_age_sex_data), axis=1)
    ...
    # X_train_clustered_without_age_sex_with_y = np.concatenate(
    #     (X_train_clustered_without_age_sex, y_train_new.reshape(-1, 1)), axis=1)

    X_train_clustered_without_age_sex_with_y = np.concatenate(
        (X_train_clustered, y_train_new.reshape(-1, 1)), axis=1)
    ...
    X_train_selected_with_age_sex = X_train_selected_without_age_sex
    ...
    # X_test_selected = np.concatenate(
    #     (X_test_selected, test_age_sex_data), axis=1)
        ...
        if classifier == 0:
            ...
            for weighting in weightings:
                ...
                fold_row = {'fold': fold, 'num_P': num_P, 'num_S': num_S, 'num_Features': nFeatures,
                            'classifier': classifier, 'weighting': weighting, 'columns_selected': column_indices, 'pred_y': pred_y, 'act_y': y_test_selected, 'test_index': train_outer_index[test_index[fold]]}
...
if __name__ == "__main__":
    ...
    subtype_labels_fold = subtype_labels[outer_train_index]
    print(subtype_labels_fold)
    # split inner folds
    inner_cv_fold = 12
    skf_inner = StratifiedKFold(
        n_splits=inner_cv_fold, shuffle=True, random_state=1000)

    inner_train_indexes = []
    inner_test_indexes = []

    for train_outer, test_outer in skf_inner.split(X_fold, subtype_labels_fold):
    ...
    param_grid = create_param_grid(
        3, 3, num_Features_upper, n_feature_min=num_Features_lower)

    columns_fold_results = ['fold', 'num_P', 'num_S', 'num_Features', 'classifier',
                            'weighting', 'columns_selected', 'pred_y', 'act_y', 'test_index']
    results_for_each_fold = pd.DataFrame(columns=columns_fold_results)

    columns_param_results = ['num_P', 'num_S', 'num_Features', 'classifier',
                             'weighting', 'pred_y', 'act_y', 'test_index', 'precision_score']
    results_for_each_param = pd.DataFrame(columns=columns_param_results)

    columns_good_results = ['num_P', 'num_S', 'num_Features', 'classifier',
                            'weighting', 'pred_y', 'act_y', 'test_index', 'precision_score']
    ...
    for param in param_grid:
        ...
        results = Parallel(n_jobs=-1)(delayed(integrated_grid_search)(X_fold, Y_fold, subtype_labels_fold, dataset, inner_train_indexes,
        ...
        for i in cw:
            ...
            if isGoodResult(0.8, row_param.get('precision_score')) == True:
                good_results = good_results.append(
                    row_param, ignore_index=True)

            results_for_each_param = results_for_each_param.append(
                row_param, ignore_index=True)
        end = time.time()
